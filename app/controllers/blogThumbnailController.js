...
exports.uploadThumbnail = async (req, res) => {
  ...
  Blog.findOne({
    ...
    if (blog && file) {
      ...
      if (imageService.validateImage(storyImage)) {
        //Validating image

        await uploadStoryThumbnail(storyImage, blog); //uploading thumbnail
        uploadStoryImage(storyImage, blog, () => {
          // uploading true size image
...
async function uploadStoryThumbnail(storyImage, blog) {
  ...
  if (width > 300) {
    ...
    story_thumb = await imageService.saveImage(
      blog.blogger_id,
      storyImage.data,
      null,
      width1,
...
async function uploadStoryImage(storyImage, blog, callback) {
  const story_thumb = await imageService.saveImage(
    blog.blogger_id,
    storyImage.data
  );
